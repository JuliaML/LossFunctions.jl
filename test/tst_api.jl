function test_vector_value(l::MarginLoss, t, y)
    ...
    @test @inferred(value!(buf2, l, t, y, AvgMode.None())) == ref
    @test buf2 == ref
    buf3 = fill!(similar(ref), 0)
    @test @inferred(value!(buf3, l, yt, AvgMode.None())) == ref
    @test buf3 == ref
    @test @inferred(value(l, t, y, AvgMode.None())) == ref
    ...
    # Sum
    s = sum(ref)
    @test @inferred(value(l, t, y, AvgMode.Sum())) ≈ s
    @test @inferred(value(l, yt, AvgMode.Sum())) ≈ s
    @test @inferred(l(t, y, AvgMode.Sum())) ≈ s
    @test @inferred(l(yt, AvgMode.Sum())) ≈ s
    # Mean
    m = mean(ref)
    @test @inferred(value(l, t, y, AvgMode.Mean())) ≈ m
    @test @inferred(value(l, yt, AvgMode.Mean())) ≈ m
    ...
    k = size(t, ndims(t))
    ## Weighted Mean
    @test_throws DimensionMismatch value(l, t, y, AvgMode.WeightedMean(ones(n-1)), ObsDim.First())
    @test_throws DimensionMismatch value(l, yt, AvgMode.WeightedMean(ones(n-1)), ObsDim.First())
    @test @inferred(value(l, t, y, AvgMode.WeightedMean(ones(n)), ObsDim.First())) ≈ m
    @test @inferred(value(l, yt, AvgMode.WeightedMean(ones(n)), ObsDim.First())) ≈ m
    @test @inferred(value(l, t, y, AvgMode.WeightedMean(ones(k)), ObsDim.Last())) ≈ m
    @test @inferred(value(l, yt, AvgMode.WeightedMean(ones(k)), ObsDim.Last())) ≈ m
    @test @inferred(value(l, t, y, AvgMode.WeightedMean(ones(k)))) ≈ m
    @test @inferred(value(l, yt, AvgMode.WeightedMean(ones(k)))) ≈ m
    ## Weighted Sum
    @test_throws DimensionMismatch value(l, t, y, AvgMode.WeightedSum(ones(n-1)), ObsDim.First())
    @test_throws DimensionMismatch value(l, yt, AvgMode.WeightedSum(ones(n-1)), ObsDim.First())
    @test @inferred(value(l, t, y, AvgMode.WeightedSum(ones(n)), ObsDim.First())) ≈ s
    @test @inferred(value(l, yt, AvgMode.WeightedSum(ones(n)), ObsDim.First())) ≈ s
    @test @inferred(value(l, t, y, AvgMode.WeightedSum(ones(k)), ObsDim.Last())) ≈ s
    @test @inferred(value(l, yt, AvgMode.WeightedSum(ones(k)), ObsDim.Last())) ≈ s
    @test @inferred(value(l, t, y, AvgMode.WeightedSum(ones(k)))) ≈ s
    @test @inferred(value(l, yt, AvgMode.WeightedSum(ones(k)))) ≈ s
    if typeof(t) <: AbstractVector
        @test_throws ArgumentError value(l, t, y, AvgMode.Sum(), ObsDim.First())
        @test_throws ArgumentError value(l, t, y, AvgMode.Mean(), ObsDim.First())
    else
        # Sum per obs
        sv = vec(sum(ref, dims=1:(ndims(ref)-1)))
        @test @inferred(value(l, t, y, AvgMode.Sum(), ObsDim.Last())) ≈ sv
        @test @inferred(value(l, yt, AvgMode.Sum(), ObsDim.Last())) ≈ sv
        buffer1 = fill!(similar(sv), 0)
        @test @inferred(value!(buffer1, l, t, y, AvgMode.Sum(), ObsDim.Last())) ≈ sv
        @test buffer1 ≈ sv
        buffer2 = fill!(similar(sv), 0)
        @test @inferred(value!(buffer2, l, yt, AvgMode.Sum(), ObsDim.Last())) ≈ sv
        @test buffer2 ≈ sv
        # Weighted sum compare
        @test @inferred(value(l, t, y, AvgMode.WeightedSum(1:k), ObsDim.Last())) ≈ sum(sv .* (1:k))
        @test @inferred(value(l, yt, AvgMode.WeightedSum(1:k), ObsDim.Last())) ≈ sum(sv .* (1:k))
        @test round(@inferred(value(l, t, y, AvgMode.WeightedSum(1:k,normalize=true), ObsDim.Last())), digits=3) ≈ 
            round(sum(sv .* ((1:k)/(sum(1:k)))), digits=3)
        @test round(@inferred(value(l, yt, AvgMode.WeightedSum(1:k,normalize=true), ObsDim.Last())), digits=3) ≈ 
            round(sum(sv .* ((1:k)/(sum(1:k)))), digits=3)
        # Mean per obs
        mv = vec(mean(ref, dims=1:(ndims(ref)-1)))
        @test @inferred(value(l, t, y, AvgMode.Mean(), ObsDim.Last())) ≈ mv
        @test @inferred(value(l, yt, AvgMode.Mean(), ObsDim.Last())) ≈ mv
        buffer3 = fill!(similar(mv), 0)
        @test @inferred(value!(buffer3, l, t, y, AvgMode.Mean(), ObsDim.Last())) ≈ mv
        @test buffer3 ≈ mv
        buffer4 = fill!(similar(mv), 0)
        @test @inferred(value!(buffer4, l, yt, AvgMode.Mean(), ObsDim.Last())) ≈ mv
        @test buffer4 ≈ mv
        # Weighted mean compare
        @test @inferred(value(l, t, y, AvgMode.WeightedMean(1:k,normalize=false), ObsDim.Last())) ≈ sum(mv .* (1:k))
        @test @inferred(value(l, yt, AvgMode.WeightedMean(1:k,normalize=false), ObsDim.Last())) ≈ sum(mv .* (1:k))
    end
end
...
function test_vector_value(l::DistanceLoss, t, y)
    ...
    # Mean
    m = mean(ref)
    @test round(@inferred(value(l, t, y, AvgMode.Mean())), digits=5) ≈ round(m, digits=5)
    @test round(@inferred(value(l, yt, AvgMode.Mean())), digits=5) ≈ round(m, digits=5)
    ...
    @test_throws DimensionMismatch value(l, t, y, AvgMode.WeightedSum(ones(n-1)), ObsDim.First())
    @test_throws DimensionMismatch value(l, yt, AvgMode.WeightedSum(ones(n-1)), ObsDim.First())
    @test @inferred(value(l, t, y, AvgMode.WeightedSum(ones(n),normalize=false), ObsDim.First())) ≈ s
    @test @inferred(value(l, yt, AvgMode.WeightedSum(ones(n),normalize=false), ObsDim.First())) ≈ s
    @test @inferred(value(l, t, y, AvgMode.WeightedSum(ones(k),normalize=false), ObsDim.Last())) ≈ s
    @test @inferred(value(l, yt, AvgMode.WeightedSum(ones(k),normalize=false), ObsDim.Last())) ≈ s
    if typeof(t) <: AbstractVector
        @test_throws ArgumentError value(l, t, y, AvgMode.Sum(), ObsDim.First())
        @test_throws ArgumentError value(l, t, y, AvgMode.Mean(), ObsDim.First())
    else
        ...
        @test buffer2 ≈ sv
        # Weighted sum compare
        @test @inferred(value(l, t, y, AvgMode.WeightedSum(1:k,normalize=false), ObsDim.Last())) ≈ sum(sv .* (1:k))
        @test @inferred(value(l, yt, AvgMode.WeightedSum(1:k,normalize=false), ObsDim.Last())) ≈ sum(sv .* (1:k))
        # Mean per obs
        mv = vec(mean(ref, dims=1:(ndims(ref)-1)))
        @test @inferred(value(l, t, y, AvgMode.Mean(), ObsDim.Last())) ≈ mv
        @test @inferred(value(l, yt, AvgMode.Mean(), ObsDim.Last())) ≈ mv
