...
@doc doc"""
    value(loss::SupervisedLoss, target::Number, output::Number) -> Number
...
@doc doc"""
    deriv(loss::SupervisedLoss, target::Number, output::Number) -> Number
...
@doc doc"""
    deriv2(loss::SupervisedLoss, target::Number, output::Number) -> Number
...
@doc doc"""
    value_deriv(loss::SupervisedLoss, target::Number, output::Number) -> Tuple
...
for (FUN, DESC, EXAMPLE) in (
    ...
    @eval begin
        ...
        @doc """
            $($(string(FUN)))!(buffer::AbstractArray, loss:SupervisedLoss, targets::AbstractArray, outputs::AbstractArray) -> buffer
